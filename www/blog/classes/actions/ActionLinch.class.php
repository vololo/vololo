 ...
 *
 */
class ActionLinch extends Action {
	/**
	 * Главное меню
	 *
	 * @var unknown_type
	 ...
	 * @var unknown_type
	 */
	protected $sMenuItemSelect='linch';
	/**
	 * СубМеню
	 *
	 * @var unknown_type
	...
	protected function EventEdit() {
		...
		if (isset($_REQUEST['submit_topic_publish']) or isset($_REQUEST['submit_topic_save'])) {
			...
			$_REQUEST['topic_linch']=$oTopic->getLinchData();
			$_REQUEST['topic_linchurl']=$oTopic->getLinchUrl();
	...
	protected function EventSave() {
		...
		$oTopic->setLinchData(getRequest('linch_data'));
		$oTopic->setUserId($this->oUserCurrent->getId());
		$this->Topic_SaveLinch($oTopic);
	...
	protected function SubmitAdd() {
		...
		$oTopic->setUserId($this->oUserCurrent->getId());
		$oTopic->setType('linch');
		$oTopic->setTitle(getRequest('topic_title'));
		//$oTopic->setLinchData(getRequest('topic_linch'));
		$oTopic->setLinchUrl(getRequest('topic_linchurl'));
	...
	protected function SubmitEdit($oTopic) {
		...
		$oTopic->setBlogId($oBlog->getId());
		$oTopic->setTitle(getRequest('topic_title'));
		//$oTopic->setLinch(getRequest('topic_linch'));
		$oTopic->setLinchUrl(getRequest('topic_linchurl'));
		$oTopic->setTextHash(md5(getRequest('topic_text')));
		/**
		 * Получаемый и устанавливаем разрезанный текст по тегу <cut>
	...
	protected function checkTopicFields() {
		...
		/**
		 * Проверяем есть ли линч топика
		 */
		if (!func_check(getRequest('topic_linch',null,'post'),'text',1,50000) || !func_check(getRequest('topic_linchurl',null,'post'),'text',1,50000)) {
			$this->Message_AddError($this->Lang_Get('topic_create_linch_error'),$this->Lang_Get('error'));
			$bOk=false;
		}
		/**
