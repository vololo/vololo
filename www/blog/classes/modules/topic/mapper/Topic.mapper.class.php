...
class ModuleTopic_MapperTopic extends Mapper {
	...
	public function AddTopic(ModuleTopic_EntityTopic $oTopic) {
		$sql = "INSERT INTO ".Config::Get('db.table.topic')."
			...
			VALUES(?d,  ?d,	?, ?, ?,	?,	?,  ?, ?, ?d, ?d, ?d, ?, ?, ?)
		";
		if ($iId=$this->oDb->query($sql,$oTopic->getBlogId(),$oTopic->getUserId(),$oTopic->getType(),$oTopic->getTitle(),$oTopic->getLinch(),$oTopic->getLinchUrl(),
	...
	public function GetLinchDatas($t) {
		$sql = "SELECT
					u.user_login as `login`,
					l.data as `data`
				FROM
					`ls_linch` as l
					JOIN  `ls_user` AS u ON l.user_id=u.user_id
				WHERE
					l.topic_id = ?d
					AND
					LENGTH(l.data) > 4
				ORDER BY l.date ";
		$aRows=$this->oDb->select($sql,$t);
		$ret = array();
		if ($aRows) foreach ($aRows as $el) $ret[] = new classLD($el);
		return $ret;
	}

	public function GetLinchData($t, $u) {
		$sql = "SELECT `data`
				FROM
					`ls_linch`
				WHERE
					`topic_id` = ?d AND `user_id` = ?d";
		$aRows=$this->oDb->select($sql,$t, $u);
		return @$aRows[0]['data'];
	}
	...
	public function SaveLinch(ModuleTopic_EntityTopic $oTopic) {
		$sql = "SELECT COUNT(*) FROM `ls_linch` WHERE
				user_id = ?d AND topic_id = ?d
		";
		$ex = $this->oDb->query($sql,$oTopic->getUserId(), $oTopic->getId());
		$ex = @(int)$ex[0]['COUNT(*)'];
		if ($ex) {
			$sql = "UPDATE `ls_linch`
				SET
				...
				WHERE
					user_id = ?d AND topic_id = ?d
			";
			$this->oDb->query($sql,$oTopic->getLinchData(),$oTopic->getUserId(), $oTopic->getId());
		}
		else {
			$sql = "INSERT INTO `ls_linch`
				SET
					data= ?, user_id = ?d,  topic_id = ?d
			";
			$this->oDb->query($sql,$oTopic->getLinchData(),$oTopic->getUserId(), $oTopic->getId());
		}
		return true;
