...
class ModuleTopic extends Module {
	...
	public function GetTopicsAdditionalData($aTopicId,$aAllowData=array('user'=>array(),'blog'=>array('owner'=>array(),'relation_user'),'vote','favourite','comment_new')) {
		 ...
		 */
		foreach ($aTopics as $oTopic) {
			$ld = $this->oMapperTopic->GetLinchDatas($oTopic->getId());
			$oTopic->setLinchDatas($ld);

			$ld = $this->oMapperTopic->GetLinchData($oTopic->getId(), $oTopic->getUserId());
			$oTopic->setLinchData($ld);
	...
	public function SaveLinch(ModuleTopic_EntityTopic $oTopic) {
		$this->oMapperTopic->SaveLinch($oTopic);
	}
	/**
	 * Обновляет топик
	 *
