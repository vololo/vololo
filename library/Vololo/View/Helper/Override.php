...
class Vololo_View_Helper_Override extends Zend_View_Helper_Abstract  {
	...
	public function overrideSingle($data, $type = null, $options = null) {
		$r = $data instanceof Vololo_View_Data ? $data : new Vololo_View_Data($data);
		$reg = Zend_Registry::isRegistered('Vololo_Multilang') ? Zend_Registry::get('Vololo_Multilang') : '';
		if ($reg && isset($reg->id)) {
		...
		if (isset($r->date)) $r->date_valid = Vololo_Common::getDate($r->date);
